...
`esy-rust` is `rust` packaged for [`esy`](https://esy.sh/).
...
`esy-rust` can be used from both NPM and directly from Github.

### From NPM 

`esy-rust` is deployed on NPM can be found
[here](https://www.npmjs.com/package/TODO).

You can simply run `esy add esy-rust` to install it, or specify it in
...
{
  "dependencies": {
    "esy-rust": "esy-packages/esy-rust"
  }
}
```
...
```diff
  "dependencies": {
+   "esy-rust": "esy-packages/esy-rust#<commit hash>"
  }
```
